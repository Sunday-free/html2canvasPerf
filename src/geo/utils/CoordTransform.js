//TODO CGCS2000-WGS84之间的坐标转换,精度有待进一步验证
/**
 * 坐标转换类。常用坐标系的转换，包括WGS84与百度坐标系(BD-09)、火星坐标系(GCJ-02)的相互转换。
 * WGS84:国际标准，Google、Bing、OSM、GPS坐标系;
 * GCJ-02：国测局坐标系，高德地图、腾讯地图坐标系;
 * BD-09：百度地图坐标系，在GCJ-02的基础上进行了二次加密;
 * @constructor
*/
function CoordTransform() {	
}

//定义一些常量
const x_PI = 3.14159265358979324 * 3000.0 / 180.0;
const PI = 3.1415926535897932384626;
const a = 6378245.0;
const ee = 0.00669342162296594323;

/**
 * 百度坐标转火星坐标
 * @param {Number} bd_lon 百度经度
 * @param {Number} bd_lat 百度纬度
 * @returns {Array<Number>} 返回火星坐标数组
 * @example
 * BOSGeo.CoordTransform.bd09togcj02(114.397433, 22.909235)
 */
 CoordTransform.bd09togcj02 =function (bd_lon, bd_lat) {
    // let bd_lon = +bd_lon;
    // let bd_lat = +bd_lat;
    let x = bd_lon - 0.0065;
    let y = bd_lat - 0.006;
    let z = Math.sqrt(x * x + y * y) - 0.00002 * Math.sin(y * x_PI);
    let theta = Math.atan2(y, x) - 0.000003 * Math.cos(x * x_PI);
    let gg_lng = z * Math.cos(theta);
    let gg_lat = z * Math.sin(theta);
    return [gg_lng, gg_lat];
}

/**
 * 火星坐标转百度坐标
 * @param {Number} lng 火星坐标经度
 * @param {Number} lat 火星坐标纬度
 * @returns {Array<Number>} 返回百度坐标数组
 * @example
 * BOSGeo.CoordTransform.gcj02tobd09(114.397433, 22.909235)
 */
 CoordTransform.gcj02tobd09 = function (lng, lat) {
    // let lat = +lat;
    // let lng = +lng;
    let z = Math.sqrt(lng * lng + lat * lat) + 0.00002 * Math.sin(lat * x_PI);
    let theta = Math.atan2(lat, lng) + 0.000003 * Math.cos(lng * x_PI);
    let bd_lng = z * Math.cos(theta) + 0.0065;
    let bd_lat = z * Math.sin(theta) + 0.006;
    return [bd_lng, bd_lat];
}

/**
 * WGS84坐标转GCj02坐标
 * @param {Number} lng 经度
 * @param {Number} lat 纬度
 * @returns {Array<Number>} 返回GCJ02坐标数组
 * @example
 * BOSGeo.CoordTransform.wgs84togcj02(114.397433, 22.909235)
 */
 CoordTransform.wgs84togcj02 = function(lng, lat) {
    // let lat = +lat;
    // let lng = +lng;
    if (out_of_china(lng, lat)) {
        return [lng, lat];
    } else {
        let dlat = transformlat(lng - 105.0, lat - 35.0);
        let dlng = transformlng(lng - 105.0, lat - 35.0);
        let radlat = lat / 180.0 * PI;
        let magic = Math.sin(radlat);
        magic = 1 - ee * magic * magic;
        let sqrtmagic = Math.sqrt(magic);
        dlat = (dlat * 180.0) / ((a * (1 - ee)) / (magic * sqrtmagic) * PI);
        dlng = (dlng * 180.0) / (a / sqrtmagic * Math.cos(radlat) * PI);
        let mglat = lat + dlat;
        let mglng = lng + dlng;
        return [mglng, mglat];
    }
}

/**
 * GCJ02坐标转WGS84坐标
 * @param {Number} lng 经度
 * @param {Number} lat 纬度
 * @returns {Array<Number>} 返回WGS84坐标数组
 * @example
 * BOSGeo.CoordTransform.gcj02towgs84(114.397433, 22.909235)
 */
CoordTransform.gcj02towgs84 = function(lng, lat) {
    // let lat = +lat;
    // let lng = +lng;
    if (out_of_china(lng, lat)) {
        return [lng, lat];
    } else {
        let dlat = transformlat(lng - 105.0, lat - 35.0);
        let dlng = transformlng(lng - 105.0, lat - 35.0);
        let radlat = lat / 180.0 * PI;
        let magic = Math.sin(radlat);
        magic = 1 - ee * magic * magic;
        let sqrtmagic = Math.sqrt(magic);
        dlat = (dlat * 180.0) / ((a * (1 - ee)) / (magic * sqrtmagic) * PI);
        dlng = (dlng * 180.0) / (a / sqrtmagic * Math.cos(radlat) * PI);
        let mglat = lat + dlat;
        let mglng = lng + dlng;
        return [lng * 2 - mglng, lat * 2 - mglat];
    }
}

/**
 * WGS84坐标转百度坐标
 * @param {Number} lng 经度
 * @param {Number} lat 纬度
 * @returns {Array<Number>} 返回百度坐标数组
 * @example
 * BOSGeo.CoordTransform.wgs84tobd09(114.397433, 22.909235)
 */
 CoordTransform.wgs84tobd09 = function(lng, lat) {
    let gcjCoords = CoordTransform.wgs84togcj02(lng, lat);
    return CoordTransform.gcj02tobd09(gcjCoords[0], gcjCoords[1]);
}

/**
 * 百度坐标转WGS84坐标
 * @param {Number} lng 经度
 * @param {Number} lat 纬度
 * @returns {Array<Number>} 返回WGS84坐标数组
 * @example
 * BOSGeo.CoordTransform.bd09towgs84(114.397433, 22.909235)
 */
 CoordTransform.bd09towgs84 = function(lng, lat) {
    let gcjCoords = CoordTransform.bd09togcj02(lng, lat);
    return CoordTransform.gcj02towgs84(gcjCoords[0], gcjCoords[1]);
}

/**
 * 计算纬度差
 * @private
 * @param {Number} lng 经度
 * @param {Number} lat 纬度
 * @returns {Number} 返回纬度差值
 */
function transformlat(lng, lat) {
    // let lat = +lat;
    // let lng = +lng;
    let ret = -100.0 + 2.0 * lng + 3.0 * lat + 0.2 * lat * lat + 0.1 * lng * lat + 0.2 * Math.sqrt(Math.abs(lng));
    ret += (20.0 * Math.sin(6.0 * lng * PI) + 20.0 * Math.sin(2.0 * lng * PI)) * 2.0 / 3.0;
    ret += (20.0 * Math.sin(lat * PI) + 40.0 * Math.sin(lat / 3.0 * PI)) * 2.0 / 3.0;
    ret += (160.0 * Math.sin(lat / 12.0 * PI) + 320 * Math.sin(lat * PI / 30.0)) * 2.0 / 3.0;
    return ret;
}

/**
 * 计算经度差
 * @private
 * @param {Number} lng 经度
 * @param {Number} lat 纬度
 * @returns {Number} 返回经度差值
 */
function transformlng(lng, lat) {
    // let lat = +lat;
    // let lng = +lng;
    let ret = 300.0 + lng + 2.0 * lat + 0.1 * lng * lng + 0.1 * lng * lat + 0.1 * Math.sqrt(Math.abs(lng));
    ret += (20.0 * Math.sin(6.0 * lng * PI) + 20.0 * Math.sin(2.0 * lng * PI)) * 2.0 / 3.0;
    ret += (20.0 * Math.sin(lng * PI) + 40.0 * Math.sin(lng / 3.0 * PI)) * 2.0 / 3.0;
    ret += (150.0 * Math.sin(lng / 12.0 * PI) + 300.0 * Math.sin(lng / 30.0 * PI)) * 2.0 / 3.0;
    return ret;
}

/**
 * 判断是否在国内，不在国内则不做偏移
 * @private
 * @param {Number} lng 经度
 * @param {Number} lat 纬度
 * @returns {boolean}
 */
function out_of_china(lng, lat) {
    // let lat = +lat;
    // let lng = +lng;
    // 纬度3.86~53.55,经度73.66~135.05
    return !(lng > 73.66 && lng < 135.05 && lat > 3.86 && lat < 53.55);
}
/* 
let CoordTransform = {
    bd09togcj02: bd09togcj02,
    gcj02tobd09: gcj02tobd09,
    wgs84togcj02: wgs84togcj02,
    gcj02towgs84: gcj02towgs84,
    wgs84tobd09: wgs84tobd09,
    bd09towgs84: bd09towgs84,
    gcTowgs84: 'gcTowgs84',
    wgs84Togc: 'wgs84Togc',
    bdTogc: 'bdTogc',
    gcTobd: 'gcTobd'
};
 */

export default CoordTransform;