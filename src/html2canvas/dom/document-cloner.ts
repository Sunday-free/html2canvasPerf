...
export class DocumentCloner {
    ...
    constructor(
        ...
        // 判断body里面的元素id是否为BOSGeo-html2canvasCollection不是就不clone
        this.documentElement = this.cloneNode(element.ownerDocument.documentElement) as HTMLElement;
        // 执行完html2canvas插件的复制，再把标签结合的加入
        let cloneBody = this.documentElement.getElementsByTagName("body")[0];
        cloneBody.appendChild(this.cloneNode(element))
    }
    ...
    cloneNode(node: Node): Node {
        ...
        if (window && isElementNode(node) && (isHTMLElementNode(node) || isSVGElementNode(node))) {
            ...
            for (let child = node.firstChild; child; child = child.nextSibling) {
                // 如果是body就退出，只有复制body外的元素，BOSGeo-html2canvasCollection内的元素复制完再增加进去
